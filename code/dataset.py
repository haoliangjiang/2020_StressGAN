...
class StressDataset(data.Dataset):
    ...
    def __init__(
        ...
        if split == "train":
            self.data = [
                (
                    sample[: self.mesh_size**2 * self.condition_nc]
                    .reshape(
                        (self.mesh_size, self.mesh_size, self.condition_nc), order="F"
                    )
                    .transpose(1, 0, 2),
                    sample[self.mesh_size**2 * self.condition_nc :].reshape(
            ...
            if not self.ignore_zero:
                self.data = [
                    (
                        sample[: self.mesh_size**2 * self.condition_nc]
                        .reshape(
                            (self.mesh_size, self.mesh_size, self.condition_nc),
                            order="F",
                        )
                        .transpose(1, 0, 2),
                        sample[self.mesh_size**2 * self.condition_nc :].reshape(
    ...
    def __getitem__(self, index):
        ...
        condition_tensor = torch.from_numpy(condition.astype(np.float32)).permute(
            2, 0, 1
        )
        stress_tensor = torch.from_numpy(
            np.array([real_stress * self.amp]).astype(np.float32)
        )
    ...
    def pad(self, nc, sample):
        ...
        if nc == 1:
            container = np.zeros((self.padded_size, self.padded_size)).astype(
                np.float32
            )
            container[: sample.shape[0], : sample.shape[1]] = sample
        else:
            container = np.zeros((self.padded_size, self.padded_size, nc)).astype(
                np.float32
            )
            container[: sample.shape[0], : sample.shape[1], :] = sample
    ...
    def visualization(self, ndata, random=False, data_idx=[]):
        ...
        for iidx, idx in enumerate(index):
            ...
            print_list = [
                condition[i].reshape(self.mesh_size, self.mesh_size)
            ...
            for j, data in enumerate(print_list):
                ax = plt.subplot(
                    ndata, self.condition_nc + 1, iidx * (self.condition_nc + 1) + j + 1
                )
                data_stress = data
