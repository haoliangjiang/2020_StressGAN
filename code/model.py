...
class Generator(nn.Module):
    ...
    def __init__(
        ...
        upsample.append(
            UpSampleFeature(
                512, out_nc, self.norm_layer, self.use_bias, k=self.k, pad=self.pad
            )
        )
        ...
        if self.k == 5 and self.pad == 2:
            ...
            last_mod = [
                nn.ReLU(True),
                nn.ConvTranspose2d(
                    ngf, outer_nc, kernel_size=self.k, stride=2, padding=self.pad
                ),
            ]
...
class VanillaDiscriminator(nn.Module):
    ...
    def __init__(
        self, input_nc, mesh_size, ndf=64, pool=None, norm_layer="batch", k=4, pad=1
    ):
        ...
        model = [
            nn.Conv2d(input_nc, ndf, kernel_size=self.k, stride=2, padding=self.pad),
            norm(ndf, affine=affine, track_running_stats=track_running_stats),
        ...
        for i in range(0, nlayer - 1):
            ...
            model += [
                ...
                norm(out_nc, affine=affine, track_running_stats=track_running_stats),
                nn.LeakyReLU(0.2, True),
            ]
